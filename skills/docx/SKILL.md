---
name: docx-legacy-fallback
description: "Legacy fallback for Microsoft Word `.docx` inspection or repair only when the official `documents:documents` route is unavailable, fails setup, or the task explicitly needs low-level OOXML surgery. Do not select for normal Word creation, editing, redlining, comments, or review."
---

## When to Use

Prefer the official `documents` skill for normal Word/`.docx` creation, editing, redlining, commenting, and visual verification. Use this legacy skill only when the official workflow is unavailable or the task specifically requires low-level OOXML inspection/repair.
