...
- **Red-Team Review:** Attack a consequential existing plan before approval. Use `adversarial-review` when the user asks for 红队审查, 失败倒推, 证伪, premortem, or when a Solution Plan contains high-cost assumptions that should be tested before implementation.
...
7. **Close the loop.** For substantial work, continue through the next artifact requested by the user: solution plan, prototype notes, test cases, rollout checklist, or recurring-task specification. Do not stop at an intermediate artifact unless the workflow has an explicit review checkpoint.
...
When the user asks for a PRD, product plan, solution plan, grey-release product definition, complaint workflow definition, or a combined product-plan plus prototype deliverable, this skill must run before drafting. A pure UI implementation or visual redesign belongs to the UI/product-design router; use this skill as well only when product behavior, scope, or a PRD is part of the request.
...
Before recommending a high-cost, hard-to-reverse, compliance-sensitive, or broadly repeated Solution Plan, invoke `adversarial-review` red-team mode. Include the resulting minimum falsification experiment and decision in the方案. Do not apply this gate to routine, reversible changes where the experiment would cost more than the change.
...
3. Ask all currently known blocking questions in one compact batch, normally no more than 3. Do not create a sequence of question-only turns.
4. Give a recommended answer and the consequence of each real choice when a decision is required.
5. If sources or low-risk assumptions resolve the gaps, continue directly. After the user's answer, update the PRD in one pass instead of asking the next predictable question.
