...
## Fourth Batch: Adversarial Quality Gates

Source reviewed: a WeChat article describing pure black-box user walkthroughs and red-team plan review with GPT-5.6. The useful part is the separation between implementation-aware checks and outside-in falsification, not the article's product claims or copied prompt wording.

### Freeze Outside-In Evidence Before Diagnosis

For a live product, first record what a new user can actually complete without reading implementation details. Freeze those findings before code inspection so implementation knowledge cannot explain away confusion.

Better behavior:
- Exercise real visible flows, including recovery and state persistence.
- Separate observed defects from unverified paths.
- Rerun the same journey after fixes.

Avoid:
- Calling screenshots, DOM presence, or a code-aware happy path a black-box acceptance test.
- Giving the evaluator known defects or intended fixes.

### Falsify Expensive Plans Before Scaling Them

Before broad global behavior changes or costly product plans, assume failure and identify the smallest experiment that could disprove the proposal.

Better behavior:
- Attack unverified assumptions, simpler alternatives, version drift, operating cost, reversibility, and blast radius.
- End with one executable recommendation and a pass/fail threshold.
- Skip the gate for low-cost reversible work.

Avoid:
- Turning red-team review into a generic risk list that cannot change the decision.
- Forcing every task through the same heavyweight process.
