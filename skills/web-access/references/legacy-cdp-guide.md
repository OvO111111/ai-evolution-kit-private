---
name: web-access
description: "Universal web access workflow for search, fetch, browser automation, CDP-style page inspection, screenshots, and login-state browser tasks when available."
---
...
- Windows 上若普通 Chrome 进程没有真正退出或端口不监听，可用隔离 profile 启动：`powershell -ExecutionPolicy Bypass -File {baseDir}/scripts/start-chrome-debug.ps1`。
...
- 若目标 URL 或用户请求命中已知站点，先执行 `scripts/match-site.sh "<user request or URL>"` 读取 `references/site-patterns/` 下的站点经验，再选择工具。
