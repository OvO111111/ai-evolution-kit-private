...
```powershell
C:\Users\skzjc\.agent-reach\venvs\wechat-article\Scripts\python.exe C:\Users\skzjc\.agent-reach\tools\wechat-article-for-ai\main.py "<URL>"
```

3. If the helper saves debug HTML but parser post-processing fails, extract manually from:

```text
C:\Users\skzjc\.agent-reach\tools\wechat-article-for-ai\output\debug\
```
