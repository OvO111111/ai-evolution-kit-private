...
async function getWebSocketUrl(port, wsPath) {
  ...
  // Modern Chrome exposes the browser-level endpoint with a UUID through
  // /json/version. Connecting to /devtools/browser without the UUID fails on
  // recent builds even though the debugging port is reachable.
  try {
    ...
    console.warn('[CDP Proxy] 无法读取 /json/version，回退到默认 WebSocket 路径:', e.message);
  }
