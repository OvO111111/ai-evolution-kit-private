---
name: pptx-legacy-fallback
description: "Legacy fallback for Microsoft PowerPoint `.pptx` inspection or repair only when the official `presentations:Presentations` route is unavailable, fails setup, or the task explicitly needs low-level PPTX or OOXML surgery. Do not select for normal presentation creation, editing, template work, or visual QA."
---

## When to Use

Prefer the official `Presentations` skill for normal PowerPoint creation, editing, template work, and visual QA. Use this legacy skill only when the official workflow is unavailable or the task specifically requires low-level PPTX/OOXML inspection/repair.
