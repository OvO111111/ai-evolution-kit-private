---
name: browser-cdp-fallback
description: Raw CDP browser-control fallback for automation, scraping, testing, or interaction only when the official `chrome:control-chrome`, in-app browser, or Playwright routes are unavailable or a confirmed low-level CDP requirement remains. Do not select merely because a task mentions Chrome, login state, or a web page.
---
