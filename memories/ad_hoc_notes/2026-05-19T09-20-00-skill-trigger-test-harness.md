...
- Added a global skill gate to `%USERPROFILE%\.codex\AGENTS.md`: every non-trivial task must classify the task family, name selected skills or state why no skill applies, identify required source inspection, and stop if a mandatory skill has not been opened or applied.
- Added mandatory PRD/product/prototype routing to `pm-prd`.
- Added `%USERPROFILE%\.codex\private\knowledge-vault\config\skill-trigger-tests.jsonl` with 25 representative task-family checks.
- Added `%USERPROFILE%\.codex\private\knowledge-vault\scripts\check_skill_routing.py` to validate expected skills against the portfolio tiers.

Validation:

- Command: `python %USERPROFILE%\.codex\private\knowledge-vault\scripts\check_skill_routing.py`
